...
if($relief_type != '' && $severity_level != '' && $status_level != ''){
    $stmt = $conn->prepare("SELECT * FROM reliefdetails WHERE TypeofRelief = ? AND FloodSeverityLevel = ? AND Status = ? AND UserID = ?");
    $stmt->bind_param("sssi",$relief_type,$severity_level,$status_level,$UserID);
} 
elseif($relief_type != '' && $severity_level != ''){
    $stmt = $conn->prepare("SELECT * FROM reliefdetails WHERE TypeofRelief = ? AND FloodSeverityLevel = ? AND UserID = ?");
    $stmt->bind_param("ssi", $relief_type,$severity_level,$UserID);
} 
elseif($relief_type != ''&& $status_level != ''){
    $stmt = $conn->prepare("SELECT * FROM reliefdetails WHERE TypeofRelief = ? AND Status = ? AND UserID = ?");
    $stmt->bind_param("ssi", $relief_type,$status_level,$UserID);
} 
elseif($severity_level != '' && $status_level != ''){
    $stmt = $conn->prepare("SELECT * FROM reliefdetails WHERE FloodSeverityLevel = ? AND Status = ? AND UserID = ?");
    $stmt->bind_param("ssi",$severity_level,$status_level,$UserID);
} 
elseif($relief_type != ''){
     $stmt = $conn->prepare("SELECT * FROM reliefdetails WHERE TypeofRelief = ? AND UserID = ?"); 
     $stmt->bind_param("si", $relief_type,$UserID);
}
elseif($severity_level != ''){
     $stmt = $conn->prepare("SELECT * FROM reliefdetails WHERE FloodSeverityLevel = ? AND UserID = ?"); 
     $stmt->bind_param("si", $severity_level,$UserID);
}
elseif($status_level != ''){
     $stmt = $conn->prepare("SELECT * FROM reliefdetails WHERE Status = ? AND UserID = ?");
     $stmt->bind_param("si",$status_level,$UserID); 
}
else {
...
<body class="bg-light">
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item">
                        <a class="nav-link" href="http://localhost/FullStackProject/User/dashboard.php">Dashboard</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="http://localhost/FullStackProject/User/create_request.php">Create Request</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="http://localhost/FullStackProject/User/view_requests.php">My Requests</a>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container py-5">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="http://localhost/FullStackProject/User/create_request.php" class="btn btn-primary btn-lg">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <form method="GET" action="http://localhost/FullStackProject/User/filterrelief.php">
             <div class="card-body">
...
<div class="container mt-5">
    <a href="http://localhost/FullStackProject/User/view_requests.php" class="btn btn-secondary mb-4">Reset Filter</a>
 <div class="row g-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="border-top pt-3">
                            <div class="btn-group w-100" role="group">
                                ...
                                <a href="http://localhost/FullStackProject/User/create_request.php?id=<?php echo $row['ReliefID']; ?>"
                                 class="btn btn-outline-secondary">
                                    <i class="bi bi-pencil me-1"></i>Edit
                                </a>
                                <a href="http://localhost/FullStackProject/User/deleterelief.php?id=<?php echo $row['ReliefID']; ?>"
